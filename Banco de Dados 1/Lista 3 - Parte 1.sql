...
SELECT DISTINCT nomeAlun FROM ALUNO A, MATRICULADO M, CURSO C, PROFESSOR P WHERE A.nivel = "JR" AND A.nroAlun = M.nroAlun AND C.idProf = P.IdProf AND P.nomeProf = "Ivana Teach";

-- Encontre a idade do aluno mais velho que é formado em History ou matriculado em um curso ministrado por Ivana Teach.
SELECT MAX(A.idade) FROM ALUNO A, MATRICULADO M, CURSO C, PROFESSOR P
WHERE A.formacao = "History" 
OR (A.nroAlun = M.nroAlun AND C.idProf = P.IdProf AND P.nomeProf = "Ivana Teach");

-- Encontre os nome de todos os cursos que são ministrados na sala R128 ou que têm cinco ou mais alunos matriculados. 
SELECT DISTINCT C.nome FROM CURSO C, MATRICULADO M WHERE (C.sala = 'R128'
AND C.nome = M.nomeCurso) OR M.nroAlun>=5;

-- Encontre os nomes de todos os alunos que estão matriculados em dois cursos que são ministrados no mesmo horário.
SELECT nomeAlun FROM ALUNO, MATRICULADO M1, MATRICULADO M2, CURSO C1, CURSO C2 WHERE
ALUNO.nroAlun = M1.nroAlun
AND ALUNO.nroAlun = M2.nroAlun
...
SELECT P.nomeProf from PROFESSOR P WHERE NOT EXISTS (SELECT sala FROM CURSO WHERE sala NOT IN (SELECT sala FROM CURSO C WHERE C.idProf = P.idProf));
...
SELECT DISTINCT P.nomeProf FROM PROFESSOR P WHERE 5 > (SELECT COUNT (M.nroAlun) FROM MATRICULADO M, CURSO C WHERE P.idProf = C.idProf AND M.nomeCurso = C.nome);

-- Imprima o nível e a idade média dos alunos desse nível, para cada nível
SELECT A.nivel, AVG(A.idade) FROM ALUNO A GROUP BY A.nivel;

-- Imprima o nível e a idade média dos alunos desse nível, para todos os níveis exceto JR.
SELECT A.nivel, AVG(A.idade) FROM ALUNO A WHERE A.nivel != 'JR' GROUP BY A.nivel;

-- Para cada professor que ministra cursos apenas na sala R128, imprima seu nome e o número total de cursos que ele ou ela ministra.
SELECT P.nomeProf, COUNT(*) AS "Num_Total Cursos" from PROFESSOR P NATURAL JOIN CURSO C NATURAL JOIN MATRICULADO M where C.sala = "R128" GROUP BY P.nomeProf; 

-- Encontre os nomes dos alunos não matriculados em nenhum curso.
SELECT DISTINCT A.nomeAlun FROM ALUNO A WHERE A.nroALUN NOT IN (SELECT M.nroAlun FROM MATRICULADO M);
