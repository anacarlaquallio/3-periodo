-- Liste os nomes dos departamentos, e caso existam, os nomes dos funcionarios.
SELECT DISTINCT D.nome, F.nome FROM DEPARTAMENTO D, FUNCIONARIO F, TRABALHA T WHERE D.id = T.id_depto
...
select DISTINCT	F1.nome FROM FUNCIONARIO F1, DEPARTAMENTO D WHERE D.id_gerente = F1.id 
AND F1.idade > (SELECT AVG(F.idade) FROM FUNCIONARIO F, TRABALHA T
