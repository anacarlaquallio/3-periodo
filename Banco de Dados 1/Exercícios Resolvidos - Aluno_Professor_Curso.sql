...
WHERE MATRICULADO.nroAlun = ALUNO.nroAlun AND 
...
WHERE CURSO.idProf = PROFESSOR.idProf AND 
sala = '20 AVW';
...
WHERE A.nroAlun = M.nroAlun AND 
M.nomeCurso = C.nome AND 
C.sala = '20 AVW';
...
SELECT COUNT(*) AS QtdeAluno FROM ALUNO;

-- Mostre a idade média de todos alunos.
SELECT SUM(idade)/COUNT(*), AVG(idade) FROM ALUNO;

--Mostre a idade máxima e a idade mínima de todos alunos.
...
SELECT nomeProf 
FROM PROFESSOR 
WHERE PROFESSOR.idDepto = '68' 
UNION 
SELECT nomeProf 
FROM PROFESSOR, CURSO 
WHERE PROFESSOR.idProf = CURSO.idProf AND 
CURSO.nome = 'Database Systems';
